...
def parse_procstat_output(file_path):
...
def draw_reaction_network(transformations: dict,
    ...
    try:
        if use_graphviz:
            if G.number_of_nodes() <= 10:
                pos = nx.nx_agraph.graphviz_layout(G, prog="neato")  # 小图：紧凑布局
            else:
                pos = nx.nx_agraph.graphviz_layout(G, prog="sfdp")   # 大图：稀疏布局
        else:
            # spring_layout 的 k 按节点数自适应，越小越紧
    ...
    # 颜色区分正/逆向
    edge_cols = [
        "#E24A33AA" if d["direction"] == "fwd" else "#348ABD88"
    ...
    if highlight_species:
        ...
        plt.title("Reaction Network", fontsize=12, pad=12)

    plt.axis("off")
    plt.tight_layout()

    png_path = outfile_prefix + ".png"
    graphml_path = outfile_prefix + ".graphml"
...
def main(directory, species_key,
    ...
    procstat_data = parse_procstat_output(procfile)
